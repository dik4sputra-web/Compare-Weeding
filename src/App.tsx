...
type Category = 'mua' | 'decoration' | 'mc' | 'music' | 'catering' | 'photo' | 'seserahan' | 'souvenir' | 'venue' | 'attire' | 'invitation' | 'transport' | 'ring' | 'mahar' | 'familyattire' | 'accessories'
...
const emptyFeatures = (cat: Category): Record<string, boolean> =>
  Object.fromEntries(featureKeys[cat].map(k => [k, false]))

function AddVendorModal({ category, onClose, onAdd, editVendor }: AddVendorModalProps) {
  const catMeta = categories.find(c => c.id === category)!
  const keys = featureKeys[category]
  const [form, setForm] = useState({
    ...
    features: editVendor?.features ? { ...editVendor.features } as Record<string, boolean> : emptyFeatures(category),
  })
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="px-7 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="block text-xs font-medium text-[#8c7b75] mb-1.5 uppercase tracking-wide">URL Foto</label>
              <input value={form.image} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} placeholder="https://images.unsplash.com/..." className="w-full px-4 py-2.5 rounded-xl border border-[#e8ddd8] text-sm text-[#2c2420] focus:outline-none focus:border-[#c9736a] transition-colors bg-[#fdf8f3]" />
            </div>
            <div className="col-span-2">
          ...
          <div>
            <label className="block text-xs font-medium text-[#8c7b75] mb-3 uppercase tracking-wide">Fitur & Layanan</label>
            <div className="grid grid-cols-2 gap-2">
              {keys.map(key => (
                <label key={key} className="flex items-center gap-2.5 px-3 py-2 rounded-lg border border-[#e8ddd8] cursor-pointer hover:bg-[#fdf8f3] transition-colors">
                  <input
                    type="checkbox"
                    checked={!!form.features[key]}
                    onChange={e => setForm(f => ({ ...f, features: { ...f.features, [key]: e.target.checked } }))}
                    className="accent-[#c9736a] w-4 h-4"
                  />
                  <span className="text-xs text-[#2c2420]">{key}</span>
                </label>
              ))}
            </div>
          </div>
...
function getSummary(vendors: Vendor[], category: Category): { id: string; badge: string; color: string; bg: string; border: string; reason: string }[] {
  const keys = featureKeys[category]
...
function SummarySection({ vendors, category, budgetNum = 0 }: { vendors: Vendor[]; category: Category; budgetNum?: number }) {
  if (vendors.length < 2) return null
  const summaries = getSummary(vendors, category)
...
function ComparisonTable({ vendors, category }: { vendors: Vendor[]; category: Category }) {
  const keys = featureKeys[category]
  return (
    ...
    <div className="min-h-screen" style={{ background: 'var(--color-cream, #fdf8f3)' }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div>
            ...
            <p className="text-sm text-[#8c7b75] mt-0.5">
              {selectedInCategory.length === 0
                ? 'Klik kartu vendor untuk memilih dan membandingkan (maks. 5)'
                : selectedInCategory.length === 1
                ? `${selectedInCategory.length} dipilih — pilih minimal 1 lagi untuk melihat perbandingan`
                : selectedInCategory.length < 5
                ? `${selectedInCategory.length} dipilih — bisa tambah hingga ${5 - selectedInCategory.length} vendor lagi`
        ...
        {selectedVendors.length >= 2 && (
          <div className="mt-8">
            <SummarySection vendors={selectedVendors} category={activeCategory} budgetNum={budgetNum} />
            <div className="rounded-2xl border border-[#e8ddd8] overflow-hidden bg-white shadow-sm">
              ...
              <ComparisonTable vendors={selectedVendors} category={activeCategory} />
            </div>
          </div>
        )}
        ...
        {sorted.length === 0 && (
          <div className="text-center py-20">
            <div className="text-5xl mb-4">{categories.find(c => c.id === activeCategory)?.icon}</div>
